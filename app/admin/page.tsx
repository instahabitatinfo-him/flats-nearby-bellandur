...
export default function AdminPage() {
  ...
  async function loadPendingProperties() {
    ...
    setPhotos((photoData || []) as PropertyPhoto[]);
  ...
  async function loadApprovedProperties() {
    ...
    const ids = approved.map((property) => property.id);
    ...
    setApprovedPhotos((photoData || []) as PropertyPhoto[]);
  ...
  async function loadEnquiries() {
    ...
    try {
      ...
      setEnquiries(
        (result.enquiries || []) as CustomerEnquiry[]
      );
      return;

      /*
       * STEP 1
       * Load the enquiry records only.
       *
       * We intentionally do not use Supabase relationship joins here.
       * Customer and property information is loaded separately below.
       */
      const {
        data: enquiryData,
        error: enquiryError,
       ...
       */
      let propertyData: EnquiryProperty[] = [];
        ...
        if (propertyError) {
          ...
          setError(
            propertyError?.message ||
              "Unable to load enquiry properties."
          );
          return;
        }

        propertyData =
          (data || []) as EnquiryProperty[];
      }
      ...
      /*
       * STEP 7
       * Combine enquiries + customer + property.
       */
  ...
  function handleAdminCheckError(error: unknown) {
    console.error(
      "ADMIN CHECK ERROR:",
      error
    );
  ...
  useEffect(() => {
    if (isAdmin && activeSection === "enquiries") {
      const enquiryTimer = window.setTimeout(() => {
        loadEnquiries();
      }, 0);

      return () => {
        window.clearTimeout(enquiryTimer);
      };
    }
  ...
  async function removeProperty(
    ...
    try {
      /*
       * Remove the actual uploaded files through
      ...
      if (error) {
        ...
        setError(
          `Unable to remove property: ${error.message}`
        );
                                  ...
                                  <>
                                    ...
                                    {enquiry.proposed_visit_date && (
                                      <p className="text-xs text-orange-600 mt-1">
                                        ...
                                        {enquiry.proposed_visit_time &&
                                          ` ${enquiry.proposed_visit_time.slice(
                                            0,
                                            5
                                          )}`}
                                      </p>
